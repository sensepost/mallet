...
public class SslServerHandler extends ChannelInitializer<Channel> {
        ...
        sniHandler = new SniHandler(mapping) {
            protected void replaceHandler(ChannelHandlerContext ctx, String hostname, SslContext sslContext)
                ...
                try {
                    ...
                    String sslHandlerName = ctx.pipeline().context(sslHandler).name();
                    ctx.pipeline().addAfter(sslHandlerName, null, new LoggingHandler("SSLTRAFFICLOGGER"));
                    sslHandler = null;
                    ctx.fireUserEventTriggered("Using certificate for " + hostname);
                } finally {
                    // Since the SslHandler was not inserted into the pipeline the ownership of the
                    // SSLEngine was not transferred to the SslHandler.
    ...
    private class TargetSslHandler extends ChannelInboundHandlerAdapter {
                    ...
                    if (cr != null) {
                        ...
                        if (sa instanceof InetSocketAddress) {
                            ...
                            if (context != null) {
                                ...
                                throw new RuntimeException("Couldn't get an SslContext for " + target);
                            }
                        } else {
                        ...
                        if (context != null) {
                            ...
                            throw new RuntimeException("Couldn't get an SslContext for " + hostname);
                    }
                }
            }
        ...
        protected void addHandler(ChannelHandlerContext ctx, String hostname, SslContext sslContext) throws Exception {
            try {
                String me = ctx.name();
                sslHandler = sslContext.newHandler(ctx.alloc(), hostname, -1);
                ...
                // ownership of the SSLEngine was not transferred to the SslHandler.
                // See https://github.com/netty/netty/issues/5678
                if (sslHandler != null) {
                    ReferenceCountUtil.safeRelease(sslHandler.engine());
                }
