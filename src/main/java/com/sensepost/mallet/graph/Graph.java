	...
	public ChannelHandler[] getNextHandlers(ChannelHandler handler, String option) {
		if (handler instanceof TargetSpecificChannelHandler) {
			if (option.endsWith(":80")) {
				...
				lookup.put(ih,
						new ChannelHandler[] { new HttpClientCodec(), new HttpObjectAggregator(1024 * 1024), ih });
				return new ChannelHandler[] { new HttpServerCodec(), new HttpObjectAggregator(1024 * 1024),
